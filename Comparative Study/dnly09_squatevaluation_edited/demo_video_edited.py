...
import numpy as np
import util
from config_reader_edited import config_reader
from scipy.ndimage import gaussian_filter
import cmu_model_edited as mod
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video_path', type=str, required=True, help='videos directory')
    ...
    for video in videos:
        ...
        output_format = '.mp4'
        video_output = output_path + '/' + video[:-4] + str(start_datetime) + output_format
        print(video_output)

        # load model
        # authors of original model don't use
        # vgg normalization (subtracting mean) on input images
        model = mod.get_testing_model()
        ...
        out = cv2.VideoWriter(video_output, fourcc, output_fps, (width, height))

        j = 0  # default is 0
        if(cam.isOpened()==False): print('Cam is not opened')
        while (cam.isOpened()) and ret_val == True and j < ending_frame:
            if j % frame_rate_ratio == 0:
                tic = time.time()
