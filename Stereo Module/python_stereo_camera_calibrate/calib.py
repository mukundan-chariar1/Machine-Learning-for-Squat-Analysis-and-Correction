...
def save_frames_single_camera(camera_name):
    ...
    while True:
        ...
        frame=cv.rotate(frame,cv.ROTATE_90_CLOCKWISE)   #code added for rotation
...
def save_frames_two_cams(camera0_name, camera1_name):
    ...
    while True:
        ...
        frame0=cv.rotate(frame0, cv.ROTATE_90_CLOCKWISE) #edited code
        frame1=cv.rotate(frame1, cv.ROTATE_90_CLOCKWISE) #edited code
...
def stereo_calibrate(mtx0, dist0, mtx1, dist1, frames_prefix_c0, frames_prefix_c1):
    ...
    for frame0, frame1 in zip(c0_images, c1_images):
        ...
        if c_ret1 == True and c_ret2 == True:
            ...
            cv.resizeWindow('img2', int(width/2), int(height/2))
            k = cv.waitKey(0)
...
def check_calibration(camera0_name, camera0_data, camera1_name, camera1_data, _zshift = 100.):
    ...
    while True:
        ...
        frame0=cv.resize(frame0, None, fx = 1/view_resize, fy=1/view_resize)
        frame1=cv.resize(frame1, None, fx = 1/view_resize, fy=1/view_resize)
...
def load_cam_intrinsics(camera_name):

    filename=camera_name+'_intrinsics.dat'
    file=os.path.join('camera_parameters',filename)

    with open(file,'r') as data:
        lines=data.readlines()
        lines2=[]
        for row in lines:
            lines2.append(row[:-2].split())
        dist=lines2[5]
        cmtx=lines2[1:4]
        ...
        cmtx0=np.asarray(cmtx0)
        dist0=np.asarray(dist0)
        return cmtx0, dist0

def load_rot_trans_data(camera_name):
    
    filename=camera_name+'_rot_trans.dat'
    file=os.path.join('camera_parameters',filename)
